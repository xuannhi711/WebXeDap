...
namespace WebXeDap.Areas.Admin.Controllers
{
	...
	public class HoadonController : Controller
	{
		...
		)
		{
			...
			if (startDate.HasValue && endDate.HasValue)
			{
				hoadons = hoadons.Where(h =>
					h.NgayLapHD >= startDate.Value && h.NgayLapHD <= endDate.Value
				);
			}
		...
		public async Task<IActionResult> DonHangMoi()
		{
			...
			var donHangMoi = hoadons.Where(hd =>
				!hd.TrangThai.Contains("Đã thanh toán") && !hd.TrangThai.Contains("Đã duyệt")
			);
		...
		public async Task<IActionResult> XacNhan(int id)
		{
			...
			if (
				!hoadon.TrangThai.Contains("Đã thanh toán")
				&& !hoadon.TrangThai.Contains("Đã duyệt")
			)
			{
