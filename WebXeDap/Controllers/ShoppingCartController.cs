	...
	public class ShoppingCartController : Controller
	{
		...
		public async Task<IActionResult> AddToCart(string masp, int sl, string mau)
		{
			...
			if (!string.IsNullOrEmpty(userId)) // Đã đăng nhập → Lưu vào database
			{
				...
				var existingCartItem = await _context.Giohangs.FirstOrDefaultAsync(c =>
					c.UserId == userId && c.MaSP == masp && c.Mau == mau
				);
		...
		public async Task<IActionResult> TangSL(string productId)
		{
			...
			{
				var cartItem = await _context.Giohangs.FirstOrDefaultAsync(c =>
					c.UserId == userId && c.MaSP == productId
				);
				if (cartItem != null)
				{
					cartItem.SoLuong++;
		...
		public async Task<IActionResult> Index()
		{
			...
			if (!string.IsNullOrEmpty(userId)) // Nếu đã đăng nhập, lấy từ database
			{
				var cartItems = await _context
					.Giohangs.Where(c => c.UserId == userId)
					.ToListAsync();
		...
		)
		{
			...
			order.Chitiethoadons = cart
				.Items.Select(i => new Chitiethoadon
				{
					MaSP = i.MaSP,
					SoLuong = i.SoLuong,
					Mausac = i.Mau,
