...
namespace WebXeDap.Areas.Controllers
{
	...
	public class SanPhamController : Controller
	{
		...
		public SanPhamController(
			ISanphamRepository sanphamRepositoryt,
		...
		)
		{
			if (ModelState.IsValid)
			{
				...
				if (SelectedColors != null && SelectedColors.Count > 0)
				{
					...
					{
						var mau = new Mau { TenMau = color, MaSP = product.MaSP };
				...
				if (Images != null && Images.Count > 0)
				{
					...
					{
						var productImage = new Anh { MaSP = product.MaSP, Url = path };
			...
			if (ModelState.IsValid)
			{
				try
				{
					...
					if (SelectedColors != null && SelectedColors.Count > 0)
					{
						foreach (var color in SelectedColors)
						{
							var mau = new Mau { TenMau = color, MaSP = sanpham.MaSP };
							await _sanphamRepository.AddMauAsync(mau);
						}
					}
					...
					if (Images != null && Images.Count > 0)
					{
						...
						foreach (var path in imagePaths)
						{
							var productImage = new Anh { MaSP = sanpham.MaSP, Url = path };
